...
use Auth;
use WebDevEtc\BlogEtc\Models\post_likes;
use Carbon\Carbon;
use Exception;
use File;
...
//    }
     ...
     * @return mixed
     */
    public function legacyApprovePost( $blogPostId)
    {
        $post = Post::findOrFail($blogPostId);
        event(new BlogPostEdited($post));
     ...
     * @return mixed
     */
    public function legacyLikePost( $blogPostId)
    {
        $post = post::findOrFail($blogPostId);
        if($post){
        event(new BlogPostEdited($post));
        $post_likes=post_likes::where('user_id',Auth::user()->id)->whereNull('deleted_at')->get();
        if($post_likes)
        {
            $post_likes->delete();
            return 'unliked';
